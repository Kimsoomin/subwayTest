...
public class PlaceOverlay extends ItemizedIconOverlay<OverlayItem> {
	...
	protected void onDrawItem(Canvas canvas, OverlayItem item, Point curScreenCoords) 
	{
		...
		} else 
		{
			...
			int widthBmp = m_bmpPosition.getWidth()/2;
			int heightBmp = m_bmpPosition.getHeight()/2;

			canvas.drawBitmap(m_bmpPosition, curScreenCoords.x - widthBmp, curScreenCoords.y - heightBmp - nOffset, null);
