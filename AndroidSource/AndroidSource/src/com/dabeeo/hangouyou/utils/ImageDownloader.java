...
public class ImageDownloader
{
	...
	public static void displayProfileImage(Context context, String url, ImageView imageView)
	{
		...
		DisplayImageOptions options = new DisplayImageOptions.Builder().showImageOnLoading(R.drawable.default_profile_s).bitmapConfig(Bitmap.Config.ARGB_8888)
				.showImageForEmptyUri(R.drawable.default_profile_s).showImageOnFail(R.drawable.default_profile_s).imageScaleType(ImageScaleType.EXACTLY_STRETCHED).cacheInMemory(false)
	...
	private static void initImageLoader(Context context)
	{
		...
		ImageLoaderConfiguration config = new ImageLoaderConfiguration.Builder(context).memoryCache(new WeakMemoryCache()).imageDecoder(new BaseImageDecoder(false))
				.discCache(new UnlimitedDiscCache(context.getFilesDir())).discCacheFileNameGenerator(new HashCodeFileNameGenerator()).build();
